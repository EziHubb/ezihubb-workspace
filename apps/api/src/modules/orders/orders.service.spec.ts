...
describe('OrdersService', () => {
        ...
        { provide: PrismaService, useValue: mockDeep<PrismaService>() },
        {
          ...
          useValue: {
            calculateShipping: jest
              .fn()
              .mockResolvedValue({ cost: 5.99, name: 'Standard Shipping' }),
            buildTrackingUrl: jest
              .fn()
              .mockReturnValue('https://tracking.example.com/123'),
          },
        },
        {
  ...
  describe('checkout', () => {
    ...
    it('creates order and returns orderId + clientSecret on happy path', async () => {
      ...
      expect(paymentsService.createPaymentIntentForOrder).toHaveBeenCalledWith(
        'order-001',
        undefined,
      );
    });
    ...
    it('throws ERR_CART_EMPTY when cart has no items', async () => {
      ...
      await expect(service.checkout(checkoutDto, 'user-001')).rejects.toThrow(
        expect.objectContaining({
          response: expect.objectContaining({ code: 'ERR_CART_EMPTY' }),
        }),
      );
    });
  ...
  describe('cancelOrder', () => {
    ...
    it('throws ERR_ORDER_CANCEL_WINDOW_EXPIRED when CONFIRMED order is older than 2 hours', async () => {
      ...
      await expect(
        service.cancelOrder('MLH-2025-00001', 'user-001'),
      ).rejects.toThrow(
        expect.objectContaining({
          response: expect.objectContaining({
    ...
    it('throws NotFoundException when order does not belong to the user', async () => {
      ...
      await expect(
        service.cancelOrder('MLH-2025-XXXXX', 'user-001'),
  ...
  describe('generateOrderNumber (via checkout)', () => {
    it('formats order number as MLH-YEAR-NNNNN with zero-padding', async () => {
      ...
      prisma.order.create.mockResolvedValue({
        ...mockOrder,
        orderNumber: `MLH-${new Date().getFullYear()}-00042`,
      } as any);
      prisma.orderItem.createMany.mockResolvedValue({ count: 1 } as any);
      prisma.orderStatusHistory.create.mockResolvedValue({} as any);
      prisma.$executeRaw.mockResolvedValue(0);
    ...
    it('produces unique numbers across sequential calls', async () => {
      ...
      prisma.order.create
        .mockResolvedValueOnce({
          ...mockOrder,
          orderNumber: `MLH-${new Date().getFullYear()}-00001`,
        } as any)
        .mockResolvedValueOnce({
          ...mockOrder,
          id: 'order-002',
          orderNumber: `MLH-${new Date().getFullYear()}-00002`,
        } as any);
