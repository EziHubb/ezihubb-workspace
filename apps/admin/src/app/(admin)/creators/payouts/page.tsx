...
function MarkPaidModal({ payout, onClose, onDone }: { payout: PayoutRow; onClose: () => void; onDone: () => void }) {
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-surface rounded-card border border-border shadow-2xl w-full max-w-[440px] p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-secondary">Mark Withdrawal as Paid</h3>
        ...
        <div>
          <label className="block text-xs font-semibold text-muted uppercase tracking-wide mb-1.5">Admin Notes <span className="font-normal text-muted/70">(optional)</span></label>
          <input type="text" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="PayPal tx #XXXXXXXXXXXX"
...
function RejectPayoutModal({ payout, onClose, onDone }: { payout: PayoutRow; onClose: () => void; onDone: () => void }) {
      ...
      <div className="bg-surface rounded-card border border-border shadow-2xl w-full max-w-[440px] p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-3 pt-1">
          <button type="button" onClick={handleReject} disabled={saving}
            ...
            <X className="w-4 h-4" />
            {saving ? 'Rejecting…' : 'Reject Withdrawal'}
          </button>
          <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-medium text-muted border border-border rounded-button hover:border-primary/40 transition-colors">Cancel</button>
        </div>
      </div>
...
export default function CreatorPayoutsPage() {
  ...
  const [payingOut, setPayingOut] = useState<PayoutRow | null>(null);
  ...
  const { data, isLoading } = useQuery<PayoutsResponse>({
    queryKey: ['admin-creator-payouts', tab, page],
  ...
  const handleModalDone = () => {
    setPayingOut(null);
    setRejecting(null);
    void qc.invalidateQueries({ queryKey: ['admin-creator-payouts'] });
  };
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-background">
                {['Creator', 'Amount', 'Method / Detail', 'Requested', 'Status', 'Actions'].map((h) => (
            ...
            <tbody className="divide-y divide-border">
                    ...
                    <tr key={p.id} className="hover:bg-muted/3 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-1">
                          ...
                          {p.adminNotes && <p className="text-[11px] text-muted font-mono truncate max-w-[160px]" title={p.adminNotes}>{p.adminNotes}</p>}
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        {(p.status === 'REQUESTED' || p.status === 'PROCESSING') && (
                          <div className="flex items-center gap-1.5">
                            <button type="button" onClick={() => setPayingOut(p)}
                              className="px-2.5 py-1 text-xs font-semibold bg-green-50 hover:bg-green-100 text-green-700 border border-green-200 rounded-button transition-colors">Mark Paid</button>
                            <button type="button" onClick={() => setRejecting(p)}
              ...
              {!isLoading && (data?.data ?? []).length === 0 && (
                <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-muted">No withdrawals with status {tab.toLowerCase()}.</td></tr>
              )}
            </tbody>
          </table>
        </div>
