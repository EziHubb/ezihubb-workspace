      ...
      <div className="bg-surface rounded-card border border-border shadow-2xl w-full max-w-[440px] p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-background rounded-card border border-border p-4 space-y-1.5">
          <p className="text-sm font-semibold text-secondary">
            {[payout.affiliate.firstName, payout.affiliate.lastName].filter(Boolean).join(' ') || payout.affiliate.email}
          </p>
...
const STATUS_TABS = ['ALL', 'REQUESTED', 'PROCESSING', 'PAID', 'REJECTED'] as const;
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-background">
                {['Affiliate', 'Amount', 'Method / Detail', 'Requested', 'Status', 'Actions'].map((h) => (
                    ...
                    <tr key={p.id} className="hover:bg-muted/3 transition-colors">
                      <td className="px-4 py-3">
                        <p className="font-medium text-secondary">
                          {[p.affiliate.firstName, p.affiliate.lastName].filter(Boolean).join(' ') || '—'}
                        </p>
