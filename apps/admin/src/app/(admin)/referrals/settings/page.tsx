...
function TierModal({
  ...
  tier:    Tier | null; // null = create new
  onClose: () => void;
  onSave:  () => void;
}) {
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            ...
            <input value={form.name} onChange={(e) => set('name', e.target.value)}
              className={inputCls} placeholder="e.g. Gold" />
          </div>

          <div>
            <FieldLabel>Min Referrals</FieldLabel>
            <input type="number" min={0} value={form.minReferrals}
              onChange={(e) => set('minReferrals', Number(e.target.value))}
              className={inputCls} />
          </div>

          <div>
            <FieldLabel>Min Earned ($)</FieldLabel>
            <input type="number" min={0} step={0.01} value={form.minEarned}
              onChange={(e) => set('minEarned', Number(e.target.value))}
              className={inputCls} />
          </div>

          <div>
            <FieldLabel>Commission Bonus (%)</FieldLabel>
            <div className="relative">
              <input type="number" min={0} max={100} step={0.1} value={form.commissionBonus}
                onChange={(e) => set('commissionBonus', Number(e.target.value))}
                className={`${inputCls} pr-7`} />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-muted text-sm">%</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <input type="color" value={form.badgeColor}
                ...
                className="h-9 w-12 border border-border rounded-button cursor-pointer p-0.5" />
          ...
          <div>
            <FieldLabel>Badge Icon <span className="font-normal text-muted/70">(emoji or short text)</span></FieldLabel>
            <input value={form.badgeIcon} onChange={(e) => set('badgeIcon', e.target.value)}
              className={inputCls} placeholder="e.g. ⭐" />
          </div>
        </div>
...
export default function ReferralSettingsPage() {
  ...
  const [s,        setS]        = useState<ReferralSettings>({
    level1Rate: 5, level2Rate: 2, level3Rate: 1,
    buyerDiscountRate: 5, buyerDiscountEnabled: true,
    minPayoutAmount: 10, lockDays: 14, isEnabled: true,
  });
  const [saving,   setSaving]   = useState(false);
  ...
  // ── Tier state ──────────────────────────────────────────────────────────────
  const [tierModal,     setTierModal]     = useState<Tier | null | false>(false); // false = closed
  const [deletingTier,  setDeletingTier]  = useState<string | null>(null);

  const { data: settingsData } = useQuery<ReferralSettings>({
    queryKey: ['admin-referral-settings'],
    queryFn:  () => api.get<ReferralSettings>(API_ROUTES.ADMIN.ADMIN_CREATORS_SETTINGS),
    staleTime: 60_000,
  });
  ...
  useEffect(() => {
    if (settingsData) setS(settingsData);
  }, [settingsData]);
  ...
  const handleDeleteTier = async (id: string) => {
    if (!await confirm('Delete this tier? Users assigned to it will have their tier cleared.', { confirmLabel: 'Delete', destructive: true })) return;
    setDeletingTier(id);
    try {
    ...
    <>
      ...
      <div className="max-w-[820px] space-y-6">
        ...
        <SectionCard title="Commission Rates" subtitle="Percentage of the order total earned at each referral level.">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            <div>
              ...
              <p className="text-xs text-muted/70 mt-1">The person who referred the buyer</p>
            </div>
            <div>
              <FieldLabel>Level 2</FieldLabel>
              <PercentInput value={s.level2Rate} onChange={(v) => set('level2Rate', v)} placeholder="2" />
              <p className="text-xs text-muted/70 mt-1">Who referred Level 1</p>
            </div>
            <div>
              <FieldLabel>Level 3</FieldLabel>
              <PercentInput value={s.level3Rate} onChange={(v) => set('level3Rate', v)} placeholder="1" />
              <p className="text-xs text-muted/70 mt-1">Who referred Level 2</p>
            </div>
          </div>
        </SectionCard>
        ...
        <SectionCard title="Buyer Discount">
          <Toggle
            checked={s.buyerDiscountEnabled}
            onChange={(v) => set('buyerDiscountEnabled', v)}
            label="Enable buyer discount on referral link"
            sub="When a customer lands via a referral link and buys, they receive a discount."
          />
        ...
        <SectionCard title="Payout Rules">
          <div className="grid grid-cols-2 gap-5">
            <div>
              ...
              <p className="text-xs text-muted/70 mt-1">Users cannot request payout below this amount</p>
            </div>
            <div>
              ...
              <input
                ...
                min={1}
                value={s.lockDays}
                onChange={(e) => set('lockDays', Number(e.target.value))}
                className={`${inputCls} w-24`}
              />
              <p className="text-xs text-muted/70 mt-1">Commissions locked for this many days before payout</p>
            </div>
          </div>
        </SectionCard>
        ...
        <SectionCard title="Programme Status">
          <Toggle
            ...
            sub="When disabled, referral codes stop generating commissions but existing balances are preserved."
          />
        </SectionCard>

        {/* Save button */}
        {saveErr && (
          <p className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-button px-3 py-2">{saveErr}</p>
        )}
        <button
          type="button"
        ...
        <div className="bg-surface rounded-card border border-border shadow-card overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            <div>
              <h4 className="font-semibold text-secondary">Tier Management</h4>
              <p className="text-xs text-muted mt-0.5">Users automatically advance tiers based on refs and earnings.</p>
            </div>
            <button
              type="button"
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-background">
                  {['Badge', 'Name', 'Min Refs', 'Min Earned', 'Bonus %', 'Order', 'Actions'].map((h) => (
                    ...
                    ? (
                        <tr>
                          <td colSpan={7} className="px-4 py-10 text-center text-sm text-muted">
                            No tiers configured. Click &ldquo;Add Tier&rdquo; to create one.
                          </td>
                        </tr>
                      )
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1.5">
                              ...
                              <button
                                ...
                                className="p-1.5 rounded-button border border-border text-muted hover:text-red-500 hover:border-red-300 transition-colors disabled:opacity-40"
                                title="Delete"
                              >
      ...
      {/* Tier modal: false = closed, null = create, Tier object = edit */}
      {tierModal !== false && (
        <TierModal
          tier={tierModal}
