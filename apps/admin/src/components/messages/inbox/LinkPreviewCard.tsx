...
export function LinkPreviewCard({ conversationId, url }: { conversationId: string; url: string }) {
  ...
  return (
    <a
      ...
      className="mt-2 flex gap-3 overflow-hidden rounded-card border border-border bg-surface p-2 hover:bg-background"
    >
      {data.image && (
        <Image
          src={data.image}
      ...
      <div className="min-w-0">
        ...
        {data.description && (
          <p className="line-clamp-2 text-xs text-muted">{data.description}</p>
        )}
      </div>
    </a>
  );
