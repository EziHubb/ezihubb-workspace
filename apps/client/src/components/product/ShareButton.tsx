...
function buildSocialLinks(t: Translator, url: string, productName: string) {
  const encoded = encodeURIComponent(url);
  const text    = encodeURIComponent(t('checkOut', { name: productName }));
  return {
...
export function ShareButton({ productSlug, productName }: ShareButtonProps) {
  ...
  const social   = buildSocialLinks(t, shareUrl, productName);
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 z-50 bg-white border border-border rounded-2xl shadow-xl w-[300px] p-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-secondary">{t('shareThisProduct')}</p>
            <button
              type="button"
